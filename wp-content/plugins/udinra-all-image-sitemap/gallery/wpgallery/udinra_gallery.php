...
$udinra_sql = "SELECT max(ID) FROM $wpdb->posts out1 WHERE out1.post_status = 'publish' AND out1.post_content like '%[gallery%' AND out1.post_type IN ($udinra_sitemap_post_type)";
$udinra_max_limit = $wpdb->get_var($udinra_sql);
$udinra_sql = "SELECT min(ID) FROM $wpdb->posts out1 WHERE out1.post_status = 'publish' AND out1.post_content like '%[gallery%' AND out1.post_type IN ($udinra_sitemap_post_type)";
...
do {
	if ($result_length == 0) {
		...
		foreach ($udinra_posts as $udinra_post) { 
			...
			include 'udinra_gallery_image.php';
		}
		if ($udinra_url_count >= $udinra_sitemap_length) {
			include 'udinra_gallery_write.php';
		}
		$udinra_min_id = $udinra_max_id + 1;
		$udinra_max_id = $udinra_max_id + 100;
		if ($udinra_max_id > $udinra_max_limit && $udinra_limit_flag == 0) {
	...
	if ( $udinra_max_id <= $udinra_max_limit) {
		$udinra_sql = "SELECT ID,post_content,post_title,post_type FROM $wpdb->posts out1 ". 
						" WHERE out1.post_status = 'publish' AND out1.post_content like '%[gallery%' ".
