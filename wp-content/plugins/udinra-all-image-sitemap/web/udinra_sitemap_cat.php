...
$udinra_sitemap_category_list = get_categories( array( 'orderby' => 'name' , 'order'   => 'ASC' ) );
 
foreach( $udinra_sitemap_category_list as $udinra_sitemap_category ) {
	$udinra_category_id = $udinra_sitemap_category->term_id;
	$udinra_sql =   " SELECT MAX(p.post_modified_gmt) AS lastmod " .
					...
					" AND term_tax.term_id = $udinra_category_id " .
	...
	$udinra_xml_image .= "\t\t"."<loc>".htmlspecialchars(get_category_link( $udinra_sitemap_category->term_id ))."</loc>".PHP_EOL;
...
if ( $udinra_image_multisite == 0) {
    ...
    $udinra_image_sitemap_url = ABSPATH . '/sitemap-image-category.xml'; 

    if (file_put_contents ($udinra_image_sitemap_url, $udinra_xml_image)) {
        $udinra_tempurl    = get_bloginfo('url').'/sitemap-image-category.xml'; 
