...
if ( get_option('woocommerce_search_clean_on_deletion') == 'yes' ) {
	...
	delete_option('wc_predictive_search_synced_categories_data');
	delete_option('wc_predictive_search_synced_tags_data');
	...
	$wpdb->query('DROP TABLE IF EXISTS ' . $wpdb->prefix . 'ps_product_categories');
	$wpdb->query('DROP TABLE IF EXISTS ' . $wpdb->prefix . 'ps_product_tags');
