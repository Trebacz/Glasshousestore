	...
	*/
	...
	public function get_result_popup() {
		...
		if ( $search_keyword != '' ) {
			...
			$product_term_id = 0;
			$post_term_id = 0;
			if ( ( ( isset( $search_in['product'] ) && $search_in['product'] > 0 ) || ( isset( $search_in['p_sku'] ) && $search_in['p_sku'] > 0 ) ) && ! empty( $cat_in ) && 'all' != $cat_in ) {
			...
			} elseif ( isset( $search_in['post'] ) && $search_in['post'] > 0 && ! empty( $cat_in ) && 'all' != $cat_in ) {
			...
			if ( isset( $search_in['p_sku'] ) && $search_in['p_sku'] > 0 ) {
				$sku_list = $wc_predictive_search->get_product_sku_results( $search_keyword, $search_in['p_sku'], 0, $product_term_id, $text_lenght, $current_lang, true, $show_price, true, false, $show_in_cat );
			...
			if ( isset( $search_in['p_cat'] ) && $search_in['p_cat'] > 0 ) {
				$header_text = wc_ps_ict_t__( 'Product Categories', __('Product Categories', 'woocommerce-predictive-search' ) );
			...
			if ( isset( $search_in['p_tag'] ) && $search_in['p_tag'] > 0 ) {
				$header_text = wc_ps_ict_t__( 'Product Tags', __('Product Tags', 'woocommerce-predictive-search' ) );
