...
class WC_PS_Term_Relationships_Data
{
	...
	public function is_newest_id() {
		...
		$is_not_newest = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT EXISTS( SELECT 1 FROM {$wpdb->term_relationships} AS tr INNER JOIN {$wpdb->term_taxonomy} AS tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE ( ( tr.object_id = %d AND tt.term_id > %d ) OR tr.object_id > %d ) AND tt.taxonomy IN ('category', 'post_tag', 'product_cat', 'product_tag') LIMIT 0,1 )",
				$latest_id,
				$latest_term_ID,
				$latest_id
			)
	...
	public function get_total_items_need_sync() {
		global $wpdb;
		return $wpdb->get_var( "SELECT COUNT( tr.object_id ) FROM {$wpdb->term_relationships} AS tr INNER JOIN {$wpdb->term_taxonomy} AS tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.taxonomy IN ('category', 'post_tag', 'product_cat', 'product_tag') " );
	}
