...
class WC_PS_Product_Categories_Data
{
	...
	/**
	 * Check Predictive Search Product Category Existed
	 */
	public function is_item_existed( $term_id ) {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT EXISTS( SELECT 1 FROM {$wpdb->ps_product_categories} WHERE term_id = %d LIMIT 0,1 )", $term_id ) );
	}
	...
	public function get_latest_post_id() {
		...
		return $wpdb->get_var( "SELECT term_id FROM {$wpdb->ps_product_categories} ORDER BY term_id DESC LIMIT 0,1" );
	}
	...
	public function get_total_items_synched() {
		...
		return $wpdb->get_var( "SELECT COUNT(term_id) FROM {$wpdb->ps_product_categories} " );
	}
	...
	public function get_total_items_need_sync() {
		...
		return $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(t.term_id) FROM {$wpdb->terms} AS t INNER JOIN {$wpdb->term_taxonomy} AS tt ON (t.term_id = tt.term_id) WHERE tt.taxonomy = %s ", 'product_cat' ) );
	}
