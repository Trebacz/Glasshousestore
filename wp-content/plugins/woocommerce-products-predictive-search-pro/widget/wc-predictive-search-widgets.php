...
class WC_Predictive_Search_Widgets extends WP_Widget 
{
	...
	function widget( $args, $instance ) {
		...
		$show_catdropdown = empty($instance['show_catdropdown']) ? 0 : $instance['show_catdropdown'];
		$show_price = empty($instance['show_price']) ? 0 : $instance['show_price'];
		$widget_template = empty($instance['widget_template']) ? 'sidebar' : $instance['widget_template'];
...
?>
		...
		<?php } ?>
            ...
            <p>
            	<label><input <?php echo ( $disabled_cat_dropdown ) ? 'disabled="disabled"' : 'name="' . $this->get_field_name('show_catdropdown').'"'; ?> type="checkbox" value="1" <?php checked( $show_catdropdown, 1 ); ?>  /> <?php _e('Search in Product Category Feature', 'woocommerce-predictive-search' ); ?></label>
            	<?php if ( $disabled_cat_dropdown ) { ?>
            	<br>
            	<span><?php echo sprintf( __( 'Activate and build <a href="%s" target="_blank">Category Cache</a> to activate this feature', 'woocommerce-predictive-search' ), admin_url( 'admin.php?page=woo-predictive-search&tab=search-box-settings&box_open=predictive_search_category_cache_box#predictive_search_category_cache_box', 'relative' ) ); ?></span>
