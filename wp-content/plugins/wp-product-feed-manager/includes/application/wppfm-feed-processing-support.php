...
//	}
	...
	protected function make_xml_string_row( $product, $category_name, $description_name, $channel ) {
		...
		$this->_channel_class->add_xml_sub_tags( $product );
...
// 060118		$clean_row_item = array_key_exists( $row_item, $row_data ) ? preg_replace( "/\r|\n/", "", $row_data[$row_item] ) : "";
	 ...
	 * @return string
	 */
	protected function make_xml_string( $key, $value, $category_name, $description_name, $google_node_pre_tag, $channel ) {
		$xml_string = '';
...
//	}
	...
	protected function add_procedural_data( &$product, $active_field_names, $selected_language ) {
		...
		if ( $woocommerce_product_parent === false ) {
		...
		if ( in_array( '_min_variation_price', $active_field_names ) ) {
			$product->_min_variation_price = $woocommerce_product_parent ? prep_money_values( $woocommerce_product_parent->get_variation_price() ) : '';
		}

		if ( in_array( '_max_variation_price', $active_field_names ) ) {
			$product->_max_variation_price = $woocommerce_product_parent ? prep_money_values( $woocommerce_product_parent->get_variation_price( 'max' ) ) : '';
		}

		if ( in_array( '_min_variation_regular_price', $active_field_names ) ) {
			$product->_min_variation_regular_price = $woocommerce_product_parent ? prep_money_values( $woocommerce_product_parent->get_variation_regular_price() ) : '';
		}

		if ( in_array( '_max_variation_regular_price', $active_field_names ) ) {
			$product->_max_variation_regular_price = $woocommerce_product_parent ? prep_money_values( $woocommerce_product_parent->get_variation_regular_price( 'max' ) ) : '';
		}

		if ( in_array( '_min_variation_sale_price', $active_field_names ) ) {
			$product->_min_variation_sale_price = $woocommerce_product_parent ? prep_money_values( $woocommerce_product_parent->get_variation_sale_price() ) : '';
		}

		if ( in_array( '_max_variation_sale_price', $active_field_names ) ) {
			$product->_max_variation_sale_price = $woocommerce_product_parent ? prep_money_values(  $woocommerce_product_parent->get_variation_sale_price( 'max' ) ) : '';
		}

		if ( in_array( 'item_group_id', $active_field_names ) ) {
			$product->item_group_id = $woocommerce_product_parent ? 'GID' . $product->ID : '';
		}

		if ( in_array( 'shipping_class', $active_field_names ) ) {
			if( $woocommerce_product_parent ) { $product->shipping_class = $woocommerce_product_parent->get_shipping_class(); }
		}
	}
