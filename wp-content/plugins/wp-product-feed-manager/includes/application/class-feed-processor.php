...
if ( !class_exists( 'WPPFM_Feed_Processor_Class' ) ) :
	...
	class WPPFM_Feed_Processor_Class extends WPPFM_Background_Process {
		...
		private function get_products_main_data( $product_id, $post_columns_query_string ) {
			...
			$meta_parent_ids = $this->get_meta_parent_ids( $product_id );
			
			array_unshift( $meta_parent_ids, $product_id ); // combine the product id with the parent ids
			...
			foreach ( $this->_pre_data['database_fields']['active_custom_fields'] as $field ) {
				$product_data->{$field} = $this->get_custom_field_data( $product_data->ID, $field );
			}

			foreach ( $this->_pre_data['database_fields']['third_party_custom_fields'] as $third_party_field ) {
				$product_data->{$third_party_field} = $this->get_third_party_custom_field_data( $product_data->ID, $third_party_field );
			}
