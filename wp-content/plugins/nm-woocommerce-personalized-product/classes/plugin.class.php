...
class NM_PersonalizedProduct extends NM_Framwork_V1 {
	...
	function setup_personalized_plugin(){
		...
		$this -> localized_vars = array (
				...
				'wc_no_decimal'			=> get_option('woocommerce_price_num_decimals'),
		);
...
<?php
	...
	function render_product_meta() {
		...
		if ($this -> productmeta_id == 0 || $this -> productmeta_id == 'None'){
			if($meta_found = $this -> check_meta_category($post->ID)){
				/**
				 * checking product against categories
				 * 
				 * @since 6.4
				 */
				$this -> productmeta_id = $meta_found;
			}else{
				return;
			}
		}
		...
		if ($existing_meta) {
			foreach ( $existing_meta as $meta ) {
				...
				if ($meta->type == 'checkbox') {
					...
					$element_value = (isset($_POST[$element_name]) ? sanitize_text_field ( $_POST [$element_name] ) : '');
					$validate_value = (isset($_POST[$validate_name]) ? $_POST[$validate_name] : '');
					if (isset($meta->required) && $meta->required == 'on' && $element_value == '' && $validate_value == 'showing') {
						$passed = false;
						$error_message = ($meta->error_message != '' ? $meta->error_message : sprintf ( __ ( '"%s" is a required field.', 'woocommerce' ), $meta->title ));
						$error_message = stripslashes ($error_message);
						nm_wc_add_notice( $error_message );
	...
	function add_product_meta_to_cart($the_cart_data, $product_id) {
		...
		//var_dump($selected_meta_id);
		
		if ($selected_meta_id == 0 || $selected_meta_id == 'None'){
			if($meta_found = $this -> check_meta_category($product_id)){
				/**
				 * checking product against categories
				 * 
				 * @since 6.4
				 */
				$selected_meta_id = $meta_found;
			}else{
		...
		if ($product_meta) {
			...
			$var_price = 0;
			$bulkquantity_price = $bulkquantity = 0;
			foreach ( $product_meta as $meta ) {
				...
				}elseif ($meta->type == 'audio') {
					
					
					$element_value = (isset($_POST [$element_name]) ? $_POST [$element_name] : '');
					if($element_value){
						$selected_media = array('type'		=> 'audio',
							'selected'	=> $element_value);
												
						//$selected_image_key = __ ( 'Image(s) selected', 'nm-personalizedproduct' );
						$product_meta_data [$meta->title] = $selected_media;
					}
					
				}elseif ($meta->type == 'bulkquantity') {
						$bulkdata = $_POST [$element_name];
						
					// check if bulkquantity is found
					if (isset($bulkdata['qty']) && intval($bulkdata['qty']) > 0) {
						$bulkdata_post = array('type'	=> 'bulkquantity',
										'data'	=> $bulkdata);
						$product_meta_data [$meta->title] =  $bulkdata_post;
						$bulkquantity_price += (isset($bulkdata['total'])) ? $bulkdata['total'] : 0 ;
						$bulkquantity = (isset($bulkdata['qty'])) ? $bulkdata['qty'] : '' ;
					}
					
				}elseif ($meta->type == 'quantities') {
				
					$provided_sizes = $_POST [$element_name];
					foreach($provided_sizes as $size => $qty) {
						if( $qty > 0 )
							$product_meta_data [$size] =  $qty;
					}
				...
				}elseif ($meta->type != 'section' && $meta->type != 'image' && $meta->type != 'audio' && $meta->type != 'quantities' && $meta->type != 'bulkquantity'){
	...
	function get_cart_session_data($cart_items, $values) {
		 ...
		 * @since 7.1
		 **/
		if( isset($values['product_meta']['bulkquantity_price']) && $values['product_meta']['bulkquantity_price'] != 0 ) {
			$cart_price = $values['product_meta']['bulkquantity_price'];
		}
			...
			if ($fixed_price){
				...
				foreach ($fixed_price as $title => $fixed){
					...
					if(isset($fixed['fee']) && $fixed['fee'] != '')
						$cart_object -> add_fee( __( esc_html($title), 'woocommerce'), floatval($fixed['fee']), $taxable );
				}
			}
	...
	function order_item_meta($item_id, $cart_item, $order_id) {
		...
		if (isset($cart_item ['product_meta']['bulkquantity']) && $cart_item ['product_meta']['bulkquantity'] != '') {
			nm_wc_add_order_item_meta ( $item_id, '_bulk_quantity', $cart_item ['product_meta']['bulkquantity'] );
		}
					...
					foreach ( $product_meta as $meta => $data ) {
						...
						if ($data -> type == 'file' || $data -> type == 'facebook') {
							...
							if ($product_files) {
								...
								$title = $data->title . " (".$item->get_name().")";
								echo '<br><hr>';
								echo '<strong>';
								printf(__('File attached %s', 'nm-personalizedproduct'), $title);
								echo '</strong>';
	...
	function crop_image(){
		...
		if (! is_wp_error ( $image )) {
			...
			$crop_params = apply_filters('nm_crop_image', array('x'=>$real_x,
																'y'=>$real_y,
																'w'=>$real_w,
																'h'=>$real_h,
																'd_w'=>NULL,
	...
	public function check_meta_category($postid) {
		
		$p_categories = get_the_terms($postid, 'product_cat');
		$meta_found = false;
		 if($p_categories){
		 	global $wpdb;
	...
	*/
	function get_all_inputs() {
		...
		$all_inputs = array (
				...
				'quantities'=> $nm_inputs->get_input ( 'quantities' ),
				'image' 	=> $nm_inputs->get_input ( 'image' ),
				'facebook'	=> $nm_inputs->get_addon ( 'facebook' ),	//Addon
				'pricematrix' => $nm_inputs->get_input ( 'pricematrix' ),
				'section' 	=> $nm_inputs->get_input ( 'section' ),
				'palettes' 	=> $nm_inputs->get_input ( 'palettes' ),
				'audio' 	=> $nm_inputs->get_input ( 'audio' )
		);
		
		// checking bulk quantity addon is enable
		include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		if ( is_plugin_active( 'ppom-addon-bulkquantity/ppom-addon-bulkquantity.php' ) ) {
			$all_inputs['bulkquantity'] = $nm_inputs->get_input ( 'bulkquantity' );
		}
		
		// checking event calendar addon is enable
		include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		if ( is_plugin_active( 'ppom-addon-eventcalendar/ppom-addon-eventcalendar.php' ) ) {
			$all_inputs['eventcalendar'] = $nm_inputs->get_input ( 'eventcalendar' );
		}
	...
	*/
	 ...
	 * @since 7.0
	 **/
	function register_meta_wpml( $meta_data ) {
		
		// nm_personalizedproduct_pa($meta_data); exit;
